...
//              }
    ...
    private fun approximateIntersectionType(
        ...
        val baseResult = when (conf.intersection) {
            TypeApproximatorConfiguration.IntersectionStrategy.ALLOWED -> if (!thereIsApproximation) {
                ...
                val alternativeType = type.getAlternativeTypeForIntersectionType()
                intersectTypes(newTypes, alternativeType, toSuper, conf, depth)
            }
    ...
    private fun intersectTypes(
        ...
        alternativeType: KotlinTypeMarker?,
        toSuper: Boolean,
        conf: TypeApproximatorConfiguration,
        depth: Int,
    ): KotlinTypeMarker {
        ...
        val alternativeTypeApproximated = if (toSuper) {
            approximateToSuperType(alternativeType, conf, depth)
        } else {
            approximateToSubType(alternativeType, conf, depth)
        } ?: alternativeType

        return createTypeWithAlternativeForIntersectionResult(intersectionType, alternativeTypeApproximated)
    }
