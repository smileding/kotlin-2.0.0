...
class Generator(
    ...
    private fun generateAbstractCheckersComponent() {
        ...
        generationPath.resolve(filename).writeToFileUsingSmartPrinterIfFileContentChanged {
            ...
            withIndent {
                ...
                for ((kClass, alias) in configuration.aliases) {
                    print("$CHECKERS_COMPONENT_INTERNAL_ANNOTATION internal val ${alias.component1().allFieldName}: ${alias.component1().setType} by lazy { ${alias.component1().fieldName}")
    ...
    private fun SmartPrinter.printDiagnosticComponentCheckMethod() {
        println("private fun <E : ${checkMethodTypeParameterConstraint.simpleName}> Collection<$abstractCheckerName<E>>.check(")
        withIndent {
            println("element: E,")
