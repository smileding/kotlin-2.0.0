...
class ResultTypeResolver(
    ...
    private fun KotlinTypeMarker.approximateToSuperTypeOrSelf(config: TypeApproximatorConfiguration): KotlinTypeMarker {
        return typeApproximator.approximateToSuperType(this, config) ?: this
    }

    private fun KotlinTypeMarker.approximateToSubTypeOrSelf(config: TypeApproximatorConfiguration): KotlinTypeMarker {
        return typeApproximator.approximateToSubType(this, config) ?: this
    }
    ...
    fun findResultTypeOrNull(
        ...
        val approximatedSubType = subType?.approximateToSuperTypeOrSelf(TypeApproximatorConfiguration.InternalTypesApproximation)
        ...
        val preparedSubType = when {
            ...
            shouldKeepCapturedTypesInSubtype(subType, approximatedSubType, superType, c) ->
                subType.approximateToSuperTypeOrSelf(TypeApproximatorConfiguration.InternalTypesApproximationNoCapturedTypes)
            else -> approximatedSubType
        }

        val preparedSuperType = when {
            approximatedSuperType == null -> null
            shouldKeepCapturedTypesInSuperType(subType, superType, approximatedSuperType, c) ->
                superType.approximateToSuperTypeOrSelf(TypeApproximatorConfiguration.InternalTypesApproximationNoCapturedTypes)
            c.isK2 && c.hasRecursiveTypeParametersWithGivenSelfType(superType.typeConstructor(c)) ->
                superType.approximateToSuperTypeOrSelf(TypeApproximatorConfiguration.InternalTypesApproximationNoCapturedTypes)
    ...
    private fun shouldKeepCapturedTypesInSubtype(
        ...
        if (!c.isK2 || superType == null || subType === approximatedSubType) return false
        if (subType.typeConstructor(c).isIntegerLiteralTypeConstructor(c)) return false
    ...
    private fun shouldKeepCapturedTypesInSuperType(
        subType: KotlinTypeMarker?,
        superType: KotlinTypeMarker,
        approximatedSuperType: KotlinTypeMarker,
        c: Context,
    ): Boolean {
        if (!c.isK2 || subType == null || superType === approximatedSuperType) return false
        if (superType.typeConstructor(c).isIntegerLiteralTypeConstructor(c)) return false

        return !AbstractTypeChecker.isSubtypeOf(c, subType, approximatedSuperType)
    }
